...
export default async function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [{ data: rows }, { data: myFollows }] = await Promise.all([
    supabase
      .from("follows")
      .select("profiles!follows_follower_id_fkey(id, username, display_name, bio, location, avatar_url, cover_url, interests, founder_badge_earned, created_at)")
        ...
        <div className="card">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-muted">People following {profile.display_name || profile.username}.</p>
            </div>
            <Link href={`/profile/${profile.username}`} className="button-secondary">Back to Profile</Link>
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          {members.length ? members.map((member) => <MemberCard key={member.username} member={member} />) : <div className="card text-sm text-muted">No followers yet.</div>}
        </div>
      </div>
    </main>
  );
